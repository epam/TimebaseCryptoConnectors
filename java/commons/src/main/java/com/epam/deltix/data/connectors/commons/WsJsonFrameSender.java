...
public class WsJsonFrameSender implements JsonWriter {
    private final StringBuilder sb = new StringBuilder();
    private final AppendableWriter<StringBuilder> json =
            new AppendableWriter<>(sb);
    private final JsonGenerator generator = new JsonGenerator(json);

    private final WebSocket webSocket;

    public WsJsonFrameSender(final WebSocket webSocket) {
    ...
    public void eoj() {
        generator.eoj();
        webSocket.sendText(json.output(), true);
        sb.setLength(0);
    }
}
